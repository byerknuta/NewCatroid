...
public class ShowTextFontAction extends Action {
    private Formula xPosition, yPosition, relativeTextSize, color, text, file, name;
    private Scope scope;
    ...
    public boolean act(float delta) {
        try {
            ...
            if (f == null || !f.exists()) {
                Log.e("ShowTextFontAction", "Font file " + fileFont + " does not exist.");
                return true;
            }

            String fontPath = f.getAbsolutePath();
            if (!fontCache.containsKey(fontPath)) {
                fontCache.put(fontPath, Typeface.createFromFile(f));
            }
            ...
            Log.d("ShowTextFontAction", "InterpretationException: " + e);
        }
        return true;
    }
